...
class DefaultTasksRepository @Inject constructor(
    ...
    override fun observeLists(): Flow<List<TaskList>> {
        return taskListsDao.observeTaskLists().map { lists ->
            lists.map(taskListMapper::toDomain)
        }
    }

    override fun observeTags(): Flow<List<Tag>> {
        return tagsDao.observeTags().map { tags ->
    ...
    override suspend fun createTask(draft: TaskDraft): Task = withContext(ioDispatcher) {
        val now = Instant.now()
    ...
    override suspend fun updateTask(task: Task): Task = withContext(ioDispatcher) {
    ...
    override suspend fun addSampleTasksIfEmpty() = withContext(ioDispatcher) {
        if (tasksDao.countTasks() > 0) {
            return@withContext
        }

        val now = Instant.now()
        val defaultList = TaskListEntity(
            id = "inbox",
            name = "Inbox",
            color = null,
            updatedAt = now,
